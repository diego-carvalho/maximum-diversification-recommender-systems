...
int main(int argc, char **argv)
{
    ...
    srand((unsigned) atoi(argv[7])); // pega a hora do relogio como semente
			...
			case 3: /* Metaheuristicas */
                ...
                switch (choice) {
                    ...
                    case 8: /* AG */
						...
						break;
                    case 9: /* AC */
						AC(n,s,p,w,b,10,10,0.5,1,5,0.9,10);
...
void AC(int n, int *s, double *p, double *w, double b, int numF, double Q, double F0, double alfa, double beta, double fator_evap, int iter_max)
{
    ...
    for (int i = 0; i < n; i++) feromonio[i] = F0;
    ...
    while (iter < iter_max){
        ...
        for (int k = 0; k < numF; k++){
            ...
            while(capResidual > 0){  
                ...
                for (int i = 0; i < n; i++)
                    if (!escolhido[i] && w[i] <= capResidual)
                        sum+= pow(feromonio[i],alfa) * pow(profit[i],beta); 
                ...
                for (int i = 0; i < n; i++){
                    if (!escolhido[i] && w[i] <= capResidual){
                        probabilidades[i] = (pow(feromonio[i],alfa) * pow(profit[i],beta) ) / sum;
                    }   
                    else{
                        probabilidades[i] = 0;
                    }    
                ...
                double *escala = (double *) malloc(n * sizeof(double));
                escala[0] = probabilidades[0];
	            //printf("escala[%2d] = %f \n",0,escala[0]);
	            for (int j=1; j < n; j++){
	    	        escala[j] = escala[j-1] + probabilidades[j];
	    	        //printf("escala[%2d] = %f \n",j,escala[j]);
	            }
	            double aux = (float)rand()/RAND_MAX;
	            int j = 0;
	            while (escala[j] <= aux) j++;
                free(escala);
            ...
            fo_pop[k] = calcula_fo(pop[k], n, p, w, b);
            //printf("Individuo %d : fo = %lf\n", k, fo_pop[k]);
            ...
            // atualiza feromonio
            for (int i = 0; i < n; i++){
                // se objeto selecionado, deixa rastro
                if (pop[k][i] == 1){
                    deltaF[i] += profit[i] * Q/fo_pop[k];
                }
            }
        }
        
        // Atualiza feromonio
        for (int i = 0; i < n; i++) feromonio[i] = feromonio[i] * fator_evap + deltaF[i];
	//getchar();
    }  
